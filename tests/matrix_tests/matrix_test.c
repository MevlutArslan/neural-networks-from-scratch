...
void test_get_sub_matrix() {
    ...
    for(int i = 0; i < 4; i++) {
        for(int j = 0; j < 4; j++) {
            source->data[i][j] = i*4 + j + 1;
        }
    }

    // Create the expected submatrix
    Matrix* expected_submatrix = create_matrix(2, 2);
    expected_submatrix->data[0][0] = 6;
    expected_submatrix->data[0][1] = 7;
    expected_submatrix->data[1][0] = 10;
    expected_submatrix->data[1][1] = 11;
...
void test_get_sub_matrix_except_column() {
    ...
    expected_submatrix->data[0][0] = 6;
    expected_submatrix->data[0][1] = 8;
    expected_submatrix->data[1][0] = 10;
    expected_submatrix->data[1][1] = 12;
    expected_submatrix->data[2][0] = 14;
    expected_submatrix->data[2][1] = 16;
